...
### 聊天室 Overlay (`/chat/overlay`)
- 極簡風格的訊息疊層，適合直接覆蓋在遊戲畫面上
- `?style=all`：顯示所有訊息
- `?style=simple`：僅顯示精簡訊息
...
2. 設定 URL 為對應功能的網址：
   ...
   - 聊天室 Overlay：`http://localhost:3000/chat/overlay?style=all`
...
```
/chat/overlay?style=all     # 顯示所有訊息
/chat/overlay?style=simple  # 精簡訊息
```
